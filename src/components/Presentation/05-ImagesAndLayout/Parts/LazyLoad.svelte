<script>
  ...
  import { lazyLoad, lazyLoadFix } from '../code-examples';
  import Range from '../../../Common/Range.svelte';
  import Intersect from '../../../Common/Intersect.svelte';
  ...
  // Intersect settings
  let activeLazyLoad = false;
  let threshold = 0.1;
  $: if (threshold) {
    show = false; // When show is true again it re-registers Intersection Observers with new threshold
  }

  function imageAttributes({ intersectionRatio, unobserve }) {
    ...
    let attr = {};
    if (fixed) {
      attr.src = `data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 ${width} ${height}\'%3E%3C/svg%3E`;
    }

    if (activeLazyLoad && intersectionRatio >= threshold) {
      attr.src = url;
      unobserve();
    }
...
<div class="container">
  <div class="code">
    <CodeBlock
      styleId="{styleId}"
      height="400px"
      code="{fixed ? lazyLoadFix : lazyLoad}"
    />
    ...
    <div class="range">
      ...
      <Range
        title="Intersect threshold"
        rotated="{false}"
        min="{0.1}"
        max="{1}"
        step="{0.1}"
        value="{threshold}"
        on:change="{(e) => (threshold = e.detail.value)}"
      />
    </div>
  </div>
  ...
  <div class="device">
    <div class="controls">
      ...
      <Button onClickHandler="{() => (activeLazyLoad = !activeLazyLoad)}">
        {activeLazyLoad ? 'Stop Lazyload' : 'Lazyload'}
      </Button>
    </div>

    <Tablet
      widthToHeightRatio="{1.5}"
      maxWidth="{tabletMaxWidth}px"
      let:scrollArea
    >
      {#if show}
        <section>
          ...
          <Intersect
            rootElement="{scrollArea}"
            threshold="{[threshold]}"
            let:intersectionRatio
            let:unobserve
          >
            <img
              {...imageAttributes({ intersectionRatio, unobserve })}
              alt=""
            />
          </Intersect>
